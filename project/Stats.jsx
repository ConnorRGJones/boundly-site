...
const StatItem = ({ value, suffix, label, sub, compact }) => {
  const [count, ref] = useCounter(value);
  return (
    <div ref={ref} style={{ padding: compact ? '20px 0' : '32px 0', textAlign: 'center' }}>
      <div style={{
        fontFamily: "'Recoleta', serif", fontWeight: 900,
        fontSize: compact ? 'clamp(26px, 7vw, 36px)' : 'clamp(40px, 5vw, 60px)',
        color: '#111111', letterSpacing: '-1px', lineHeight: 1,
      }}>{count}{suffix}</div>
      <div style={{ fontFamily: "'Inter', sans-serif", fontSize: compact ? 11 : 13, fontWeight: 600, color: '#242424', marginTop: compact ? 6 : 8 }}>{label}</div>
      {!compact && <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: '#898989', marginTop: 4, lineHeight: 1.5 }}>{sub}</div>}
    </div>
  );
};
...
const BoundlyStats = () => {
  ...
  return (
    <section style={{ background: '#fff', borderBottom: '1px solid rgba(34,42,53,0.07)' }}>
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: isMobile ? '0 12px' : '0 32px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
        {stats.map((s, i) => (
          <div key={i} style={{
            borderRight: i < 2 ? '1px solid rgba(34,42,53,0.07)' : 'none',
            padding: isMobile ? '0 4px' : '0 32px',
            textAlign: 'center',
          }}>
            <Reveal delay={i * 120}>
              <StatItem {...s} compact={isMobile} />
            </Reveal>
          </div>
        ))}
      </div>
